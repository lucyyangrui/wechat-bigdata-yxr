...
def setup_logging(mode='pretrain', save_path=None):
    if save_path == None:
        if mode == 'pretrain':
            log_f_name = 'log/' + f"train_{time.strftime('%m%d_%H%M', time.localtime())}.log"
        else:
            log_f_name = 'log/' + f"finetune_{time.strftime('%m%d_%H%M', time.localtime())}.log"
    else:
        log_f_name = save_path
