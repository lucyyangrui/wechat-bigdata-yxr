...
def build_optimizer(args, model, mode='pretrain', total_step=10000):
    if mode == 'pretrain':
        lr_dict = {'others':5e-4, 'text_encoder':5e-5}  # 5e-5
    else:
        lr_dict = {'others':5e-5, 'text_encoder':5e-5}
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=lr_dict['text_encoder'], eps=args.adam_epsilon)
