...
def create_dataloaders(args, train_index=range(90000), val_index=range(10000), pretrain=False, test_mode=False):
    if pretrain:
        batch_size = args.pre_batch_size
        ann_path = args.pretrain_annotation
        zip_feats = args.pretrain_zip_feats
    else:
        batch_size = args.batch_size
        ann_path = args.train_annotation
        zip_feats = args.train_zip_feats

    dataset = MultiModalDataset(args, ann_path, zip_feats, test_mode)
    size = len(dataset)
...
class MultiModalDataset(Dataset):
    ...
    def tokenize_text(self, text_title: str, text_ocr: str, text_asr: str) -> tuple:
        ...
        if len(text_title) > 128:
            text_title = text_title[-128:]
        if len(text_asr) > 60:
            text_asr = text_asr[-60:]
        ocr_len_max = 128 - len(text_title) + 60 - len(text_asr)
        if len(text_ocr) > ocr_len_max:
            text_ocr = text_ocr[:ocr_len_max]
        text = '[SEP]' + text_title + '[SEP]' + text_ocr + '[SEP]' + text_asr
        token_type_ids[:len(text_title) + 1] = 0
